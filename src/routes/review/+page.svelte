	...
	{:else if draft}
		...
		<section class="card-bg border rounded p-3 sm:p-4 space-y-2">
			...
			{#if liveWarnings.length > 0}
				<ul class="text-sm text-note list-disc ml-5 space-y-0.5" aria-live="polite">
		...
		{#if tab === 'source'}
			<section class="space-y-2">
				...
				<textarea
					...
					autocapitalize="off"
					autocomplete="off"
					class="w-full font-mono text-sm leading-relaxed p-3 rounded border border-paper-3 bg-paper-1 text-ink focus:ring-2 focus:ring-accent focus:border-transparent whitespace-pre overflow-x-auto"
